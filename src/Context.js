...
class Context extends Component {
  ...
  handleLogin = (event, data, history) => {
    ...
    axios
      .post("http://192.168.0.108:8000/accounts/login/", credentials)
      .then((response) => {
        this.setState(
          {
  ...
  setCheckedInRooms = () => {
    ...
    if (this.state.isAdmin) {
      axios
        .get(
          "http://192.168.0.108:8000/hotel/get_current_checked_in_rooms/",
          config
        )
        .then((response) => {
          this.setState({
  ...
  handleCheckOut = (room_id) => {
    ...
    axios
      .post("http://192.168.0.108:8000/hotel/checkout/", { pk: room_id }, config)
      .then((response) => {
  ...
  handleRegister = (event, data, history) => {
    event.preventDefault();
    axios
      .post("http://192.168.0.108:8000/accounts/register/", data)
      .then((response) => {
        history.push("/login");
      })
      .catch((error) => {
